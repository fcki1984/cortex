...
describe('Platform surface migration', () => {
  ...
  it('covers the review inbox import and delta-sync mainline in the smoke gate', () => {
    ...
    expect(smoke).toContain('auto-commit bounded english colloquial profile rules');
    expect(smoke).toContain("user_message: 'Use English from now on'");
    ...
    expect(smoke).toContain('no-op english colloquial response-length synonym');
    expect(smoke).toContain("user_message: 'Keep replies to three sentences'");
    expect(smoke).toContain("synonymEnglishLengthIngest.json?.auto_committed_count === 0");
    ...
    expect(smoke).toContain('no-op english strongest solution-complexity synonym');
    expect(smoke).toContain("user_message: 'Use the simplest approach'");
    expect(smoke).toContain("synonymEnglishComplexityIngest.json?.auto_committed_count === 0");
    expect(smoke).toContain('auto-commit colloquial chinese solution complexity');
    expect(smoke).toContain("user_message: '方案尽量简单点'");
    expect(smoke).toContain('auto-commit colloquial chinese organization fact');
    expect(smoke).toContain("user_message: '在 OpenAI 上班'");
    expect(smoke).toContain('no-op colloquial chinese current-organization synonym');
    expect(smoke).toContain("user_message: '目前在 OpenAI 上班'");
    expect(smoke).toContain("colloquialChineseExpansionRecords.json?.items || []).map((item) => item.content).sort()");
    ...
    expect(smoke).toContain('no-op english employed-organization synonym');
    expect(smoke).toContain("user_message: 'Currently employed at OpenAI'");
    expect(smoke).toContain("synonymEnglishOrganizationIngest.json?.auto_committed_count === 0");
    ...
    expect(smoke).toContain('no-op english residing-location synonym');
    expect(smoke).toContain("user_message: 'Currently residing in Tokyo'");
    expect(smoke).toContain("synonymEnglishResidenceIngest.json?.auto_committed_count === 0");
    ...
    expect(smoke).toContain('no-op english reordered explicit response-style synonym');
    expect(smoke).toContain("user_message: 'Respond directly and concisely'");
    expect(smoke).toContain("synonymEnglishResponseStyleIngest.json?.auto_committed_count === 0");
    ...
    expect(smoke).toContain('create natural retain-mission priority-task agent');
    expect(smoke).toContain("retainMission: '保留长期偏好、稳定背景和当前重点任务'");
    expect(smoke).toContain('auto-commit natural retain-mission priority task');
    expect(smoke).toContain("priorityMissionIngest.json?.mission_filtered_count === 0");
    expect(smoke).toContain("priorityMissionRecords.json?.items || []).some((item) => item.kind === 'task_state' && item.content === '当前任务是重构 Cortex recall')");
    expect(smoke).toContain('create natural retain-mission keyed agent');
    expect(smoke).toContain("retainMission: '只保留沟通语言和工作公司'");
    expect(smoke).toContain('auto-route natural keyed retain-mission import');
    expect(smoke).toContain("missionKeyedImport.json?.auto_committed_count === 2");
    expect(smoke).toContain("missionKeyedImport.json?.mission_filtered_count === 1");
    expect(smoke).toContain("missionKeyedRecords.json?.items || []).some((item) => item.attribute_key === 'language_preference' && item.content === '请用中文回答')");
    expect(smoke).toContain("missionKeyedRecords.json?.items || []).some((item) => item.attribute_key === 'organization' && item.content === '我在 OpenAI 工作')");
    expect(smoke).toContain("!(missionKeyedRecords.json?.items || []).some((item) => item.attribute_key === 'location')");
